...
public class ParallelSagaJoinInterceptor extends StateMachineInterceptorAdapter<PaymentState, PaymentEvent> {
    ...
    private void checkParallelJoinForAllRegions(StateMachine<PaymentState, PaymentEvent> rootStateMachine) {
    ...
    private void tryTriggerCompletion(
            StateMachine<PaymentState, PaymentEvent> rootStateMachine, Collection<PaymentState> ids) {
        Object previous =
                rootStateMachine.getExtendedState().getVariables().putIfAbsent(JOIN_TRIGGERED_KEY, Boolean.TRUE);
        if (previous == null) {
