...
public class StateMachineConfig extends GeneratedStateMachineConfig {
    ...
    @Override
    protected void executeAsyncSanctionsCheckAction(StateContext<PaymentState, PaymentEvent> context) {
        paymentProcessingSaga.startSanctionsCheck(context);
    }

    @Override
    protected void executeAsyncFeeCalculationAction(StateContext<PaymentState, PaymentEvent> context) {
    ...
    protected void executeSettlementErrorAction(StateContext<PaymentState, PaymentEvent> context) {
        var proxy = SagaContextProxy.of(context);
        String msg = context.getException() != null ? context.getException().getMessage() : UNKNOWN_ERROR;
