...
@Component
@Slf4j
public class PaymentStateMachinePersister implements StateMachinePersister<PaymentState, PaymentEvent, Payment> {
    ...
    public void persist(StateMachine<PaymentState, PaymentEvent> stateMachine, Payment payment) {
        String stateStr =
                stateMachine.getState().getIds().stream().map(Enum::name).collect(Collectors.joining(COMMA));
    ...
    public StateMachine<PaymentState, PaymentEvent> restore(
        ...
        log.debug(
                "[Persister] Restoring state machine to state {} for payment {}", payment.getState(), payment.getId());
        StateMachineContext<PaymentState, PaymentEvent> ctx =
                createContext(stateNames, PaymentState.valueOf(stateNames[0]), createExtendedState(payment));
        stateMachine.stop();
    ...
    private StateMachineContext<PaymentState, PaymentEvent> createContext(
            String[] stateNames, PaymentState storedState, DefaultExtendedState extendedState) {
