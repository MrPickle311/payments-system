...
class ExportJobIntegrationTest {
    ...
    private void sendTestEventToKafka(LedgerEvent event, int partition, String key) throws JsonProcessingException {
        kafkaTemplate.send(exportProperties.getTopic(), partition, key, objectMapper.writeValueAsString(event));
    }

    private void setupMockServerExpectations() {
        mockServer
                .expect(min(1), requestTo(exportProperties.getRegulatory().getUrl()))
