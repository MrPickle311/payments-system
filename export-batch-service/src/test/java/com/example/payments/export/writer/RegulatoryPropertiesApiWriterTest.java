...
class RegulatoryPropertiesApiWriterTest {
  ...
  void setUp() {
    ...
    ExportProperties.RegulatoryProperties regulatory =
        mock(ExportProperties.RegulatoryProperties.class);
    when(exportProperties.getRegulatory()).thenReturn(regulatory);
    when(regulatory.getUrl()).thenReturn(API_URL);

    writer = new RegulatoryApiWriter(restTemplate, exportProperties,
