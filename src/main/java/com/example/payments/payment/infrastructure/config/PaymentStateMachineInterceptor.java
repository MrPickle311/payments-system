...
/**
 * Audit interceptor that records every state transition into the history table.
 */
@Component
public class PaymentStateMachineInterceptor
        extends StateMachineInterceptorAdapter<PaymentState, PaymentEvent> {

    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PaymentStateMachineInterceptor.class);
    ...
    public void preStateChange(State<PaymentState, PaymentEvent> state,
        ...
        if (transition != null && transition.getSource() != null) {
            ...
            PaymentEvent event  = transition.getTrigger() != null ? transition.getTrigger().getEvent() : null;
            ...
            if (Boolean.TRUE.equals(stateMachine.getExtendedState().get("isRestoring", Boolean.class))) {
                log.debug("[Interceptor] Skipping history for restoration: payment={} {} -> {}", 
                        paymentId, source, target);
                return;
            }

            log.info("[Interceptor] Record transition for payment {}: {} --({})--> {}",
                    paymentId, source, event, target);
